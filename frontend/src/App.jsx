...
function ConfigErrorScreen({ message }) {
  return (
    <div style={{
      ...
      fontFamily: 'Inter, system-ui, sans-serif', background: '#0F172A', color: '#F8FAFB',
    }}>
      <div style={{ maxWidth: 520, textAlign: 'center', background: '#1E293B', borderRadius: 12, padding: '2.5rem 2rem', boxShadow: '0 8px 32px rgba(0,0,0,.3)' }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>⚙️</div>
        <h1 style={{ fontSize: 22, margin: '0 0 12px', fontWeight: 700 }}>Configuration Error</h1>
        <p style={{ fontSize: 14.5, lineHeight: 1.6, color: '#94A3B8', margin: '0 0 20px' }}>{message}</p>
        <div style={{ background: '#0D1F2D', borderRadius: 8, padding: '1rem 1.25rem', textAlign: 'left', fontSize: 13, color: '#64748B', fontFamily: 'monospace' }}>
          ...
          <p style={{ margin: '4px 0' }}>• REACT_APP_LANDING_URL</p>
        </div>
      </div>
    </div>
  );
    ...
    <ThemeProvider>
      <AuthProvider>
        ...
        <Toaster
          ...
          toastOptions={{
            duration: 4000,
            style: { fontFamily: 'Inter, sans-serif', fontSize: '13.5px', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,.12)' },
